...
import { ArrowUpRight, ChevronRight, Menu, Rocket, Search, Sparkles, X } from "lucide-react";
    ...
    <>
      <header className="site-header">
        <div className="page-shell flex h-16 items-center justify-between gap-3">
          <Link href="/" className="brand-lockup" aria-label="Prime Heroes Wiki home">
            <span className="brand-mark"><Rocket className="h-5 w-5" /></span>
            <span className="min-w-0">
              <span className="brand-name">Prime Heroes</span>
              <span className="brand-kicker">Roblox wiki & tracker</span>
            </span>
          </Link>
          ...
          <div className="flex items-center gap-2">
            <a href={site.officialNewsUrl} target="_blank" rel="noopener noreferrer" className="header-official">
              Official reveal <ArrowUpRight className="h-4 w-4" />
            </a>
            <button type="button" onClick={() => setSearchOpen(true)} className="header-action" aria-label="Search the wiki" title="Search the wiki">
      ...
      {searchOpen ? (
        <div className="search-overlay" role="dialog" aria-modal="true" aria-label="Wiki search">
          <div className="search-dialog">
            <div className="search-field-row">
              <Search className="h-5 w-5 shrink-0 text-lilac-700" />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search heroes, release date, lore..." className="search-field" autoFocus />
            ...
            <div className="search-results">
              ...
              )) : <p className="search-empty">No matching wiki page.</p>}
            </div>
          </div>
        </div>
      ) : null}
